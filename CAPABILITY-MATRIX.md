...
| Model selection | ❌ | Hard-locked to `gpt-4o-mini-2024-07-18` (128k context) — **but only for the in-app AI proxy** (`Chat()`/`Generate()`). Build agents (Cursor) expose a curated model set (fable-5, opus, gpt-5.6 variants, sonnet-5, composer-max); Audos Code is Claude-backed; the customer-facing agent's model is separately configurable. No selection path for the in-app proxy specifically. | `experiments/0028`, Otto chat 2026-07-23 (refinement, not independently verified) |
